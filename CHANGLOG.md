...
- Localized the main app, Settings, category flows, dialogs, toasts, and login screen so the UI shows only the selected language.
- Added CSV import next to CSV export with format validation and localized error messages.
- Improved the add/edit note form so it stays usable when the keyboard is open.
- Standardized search placeholders and added per-note sharing from list items.
- Updated release metadata to versionCode 2 and versionName 0.2.
- Fixed unit test compatibility after UI signature changes and Robolectric SDK support limits.
